...
public class GImportCommand extends ArgsCommand {
    ...
    public boolean executeConsoleCommand(ConsoleCommandSender console, String[] args) {
        return handleImport(console, args);
    }
    ...
    private boolean handleImport(CommandSender sender, String[] args) {
        ...
        if (GWorld.getInstance().getDataHandler().containsWorld(worldName)) {
            sender.sendMessage(SendMessageUtil.sendMessage("Import.failed").replace("%world%", worldName));
            return false;
        }
        ...
        if (mapping == null) {
            sender.sendMessage(SendMessageUtil.sendMessage("Import.failed"));
            sender.sendMessage(SendMessageUtil.sendMessage("Import.use"));
            return false;
        }

        WorldData worldData = new WorldData(worldName, mapping.getEnvironment(), mapping.getWorldType(), 0, generator);

        sender.sendMessage(SendMessageUtil.sendMessage("Import.creating").replace("%world%", worldName));
        ...
        sender.sendMessage(SendMessageUtil.sendMessage("Import.success").replace("%world%", worldName));
